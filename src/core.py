...
class OrganizadorArquivos:
    ...
    def executar(self):
        ...
        if not self.origem.exists():
            logging.error(f"ERRO: A pasta de origem não existe: {self.origem}")
            return
        ...
        if total == 0:
            logging.warning("--- ALERTA: NENHUM ARQUIVO ENCONTRADO ---")
            logging.warning(f"A pasta '{self.origem.name}' e suas subpastas parecem vazias.")
            return

        logging.info(f"Encontrados {total} arquivos em todas as subpastas. Iniciando...")
    ...
    def _escanear_arquivos(self):
        lista_arquivos = []
        logging.info(f"Escaneando RECURSIVAMENTE: {self.origem}...")
        logging.info("(Isso pode demorar um pouco se houver muitos arquivos...)")
        
        try:
            for item in self.origem.rglob('*'):
                try:
                    if not item.is_file():
                        continue

                    if item.name.startswith('.') or item.name == 'log_organizacao.txt':
                        continue

                    if self.destino in item.parents:
                        continue
            ...
            logging.error(f"Erro durante o escaneamento: {e}")
            ...
            if caminho_destino_previsto.exists():
                if self.simulacao:
                    ...
                    if hash_origem == hash_destino:
                        logging.warning(f"DUPLICATA: {arquivo.name} já existe. Ignorado.")
                        self.stats['duplicado'] += 1
                        return 
                    else:
                        caminho_destino_final = FileRenamer.make_unique_name(caminho_destino_previsto)
                        logging.info(f"RENOMEAR: {arquivo.name} -> {caminho_destino_final.name}")
                        self.stats['renomeado'] += 1
            
            if self.simulacao:
                logging.info(f"[SIMULAÇÃO] Moveria: {arquivo.name}")
                self.stats['sucesso'] += 1
            else:
                pasta_final.mkdir(parents=True, exist_ok=True)
                shutil.move(str(arquivo), str(caminho_destino_final))
                logging.info(f"MOVIDO: {arquivo.name}")
                self.stats['sucesso'] += 1
        except Exception as e:
